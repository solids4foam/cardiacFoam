...
`applications/scripts/driverFoam/` (the `driverFoam` CLI / `openfoam_driver`
...
(`driverFoam plan --strict`, `driverFoam run --strict`,
`driverFoam sweep-run --spec sweep.json`). Read
...
and drive it through `driverFoam` — don't add a new `run_*.sh`.
