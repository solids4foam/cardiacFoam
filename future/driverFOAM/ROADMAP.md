...
| P1.9 | P1 | Normalize manufactured-verifier summary schemas across C++ backends | `bathBidomain` no longer emits a one-off `# cellsPerDirection` / `# time` header while other manufactured verifiers emit `Number of cells` / `Final simulation time`; Python adapters and regression harnesses require no case-specific parsing branches |
