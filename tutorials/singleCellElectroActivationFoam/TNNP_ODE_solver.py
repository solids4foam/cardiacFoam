...
def solve_model():
    ...
    for (i,t) in enumerate(voi[1:]):
        if r.successful():
            r.integrate(t)
            states[:,i+1] = r.y
            sys.stdout.write('\rRunning... ' + spinner[i % len(spinner)])
            sys.stdout.flush()
        else:
            break
...
def plot_model_subplot(ax, voi, states, algebraic, selected_states=None, selected_algebraic=None):
    """Plot selected variables against variable of integration on a given subplot axis"""
    (legend_states, legend_algebraic, legend_voi, _) = createLegends()

    state_indices = selected_states if selected_states is not None else []
    alg_indices = selected_algebraic if selected_algebraic is not None else []

    data_to_plot = np.vstack((
        states[state_indices] if state_indices else np.empty((0, len(voi))),
        algebraic[alg_indices] if alg_indices else np.empty((0, len(voi)))
    )).T

    legend_labels = [legend_states[i] for i in state_indices] + [legend_algebraic[i] for i in alg_indices]

    ax.plot(voi, data_to_plot)
    ax.set_xlabel(legend_voi)
    ax.set_ylabel("Values")
    ax.legend(legend_labels, loc='best')
    ax.grid(True)

def plot_multiple_models(voi, states, algebraic, plots, ncols):
    """
    `ncols` defines number of columns; rows are computed automatically
    """
    num_plots = len(plots)
    nrows = math.ceil(num_plots / ncols)

    fig, axes = pylab.subplots(nrows, ncols, figsize=(5 * ncols, 4.5 * nrows))
    axes = np.array(axes).reshape(-1)  # Flatten in case of single row or column

    for i, plot_config in enumerate(plots):
        plot_model_subplot(axes[i], voi, states, algebraic,
                           selected_states=plot_config.get('selected_states'),
                           selected_algebraic=plot_config.get('selected_algebraic'))

    # Hide unused subplots if total plots < nrows * ncols
...
plot_configs = [
        ...
        {'selected_states': [0], 'selected_algebraic': []},
        {'selected_states': [3], 'selected_algebraic': []}
]

plot_multiple_models(voi, states, algebraic, plot_configs, ncols=2)

plot_configs = [
        {'selected_states': [0], 'selected_algebraic': []},
        {'selected_states': [3], 'selected_algebraic': []},
        {'selected_states': [], 'selected_algebraic': [53]},
        {'selected_states': [], 'selected_algebraic': [57]},
        {'selected_states': [], 'selected_algebraic': [51]},
        {'selected_states': [], 'selected_algebraic': [52]},
        {'selected_states': [], 'selected_algebraic': [55]},
        {'selected_states': [], 'selected_algebraic': [50]}
]

    # 3 columns will make 2 rows automatically
plot_multiple_models(voi, states, algebraic, plot_configs, ncols=2)
