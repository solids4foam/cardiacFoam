...
TET_TEMPLATE="$CASE_DIR/studies/mesh/tet/box.geo.template"
TET_SCHEMES="$CASE_DIR/studies/mesh/tet/fvSchemes"
OPENFOAM_BASHRC="${OPENFOAM_BASHRC:-/Volumes/OpenFOAM-v2412/etc/bashrc}"
PYTHON="${PYTHON:-python3}"
# tet   -- gmsh Delaunay meshes + this case's own studies/mesh/tet overlay
...
for n in "${RESOLUTIONS[@]}"; do
    ...
    rsync -a \
        ...
        --exclude '/studies/mesh/tet/studies/corrector/results/' \
        "$CASE_DIR/" "$run_case/"

    if [[ "$MESH_MODE" == "tet" ]]; then
        lc="$($PYTHON -c "print(1.0/float('$n'))")"
        sed "s/__LC__/$lc/" "$TET_TEMPLATE" > "$run_case/studies/box.geo"
        cp "$TET_SCHEMES" "$run_case/system/fvSchemes"
        (
            cd "$run_case"
            gmsh -3 studies/box.geo -o box.msh -format msh2 > log.gmsh 2>&1
