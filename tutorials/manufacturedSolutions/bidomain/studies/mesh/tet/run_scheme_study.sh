...
# studies/mesh/tet/fvSchemes overlay, which studies/corrector uses as its
...
_TET_GRAD="$(awk '/^gradSchemes/{f=1} f&&/default/{print $2; exit}' studies/mesh/tet/fvSchemes)"
if [ "$_SYS_GRAD" != "$_TET_GRAD" ]; then
    echo "ERROR: gradSchemes disagree: system/fvSchemes=$_SYS_GRAD studies/mesh/tet/fvSchemes=$_TET_GRAD" >&2
    exit 3
fi
...
OUT="${OUT:-studies/results/scheme_study.csv}"
export PHI_TOL
...
# sensitivity study (studies/mesh/tet/studies/corrector) already established
...
if [ "$OUT" = "studies/results/scheme_study.csv" ] \
   && [ "$PHI_TOL" = "1e-6" ] \
