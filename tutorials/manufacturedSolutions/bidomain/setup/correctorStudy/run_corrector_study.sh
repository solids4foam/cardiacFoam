...
TET_TEMPLATE="$CASE_DIR/../monodomainTetMMS/setup/box.geo.template"
TET_SCHEMES="$CASE_DIR/../monodomainTetMMS/system/fvSchemes"
OPENFOAM_BASHRC="${OPENFOAM_BASHRC:-/Volumes/OpenFOAM-v2412/etc/bashrc}"
PYTHON="${PYTHON:-python3}"
# tet   -- gmsh Delaunay meshes + the monodomainTetMMS leastSquares schemes.
...
for n in "${RESOLUTIONS[@]}"; do
    ...
    rsync -a \
        ...
        --exclude '/setup/correctorStudy/results/' \
        "$CASE_DIR/" "$run_case/"

    if [[ "$MESH_MODE" == "tet" ]]; then
        lc="$($PYTHON -c "print(1.0/float('$n'))")"
        sed "s/__LC__/$lc/" "$TET_TEMPLATE" > "$run_case/box.geo"
        cp "$TET_SCHEMES" "$run_case/system/fvSchemes"
        (
            cd "$run_case"
            gmsh -3 box.geo -o box.msh -format msh2 > log.gmsh 2>&1
