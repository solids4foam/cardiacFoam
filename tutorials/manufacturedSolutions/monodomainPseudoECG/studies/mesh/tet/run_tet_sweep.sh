...
# Merged case (monodomainPseudoECG): activate the tet fvSolution overlay
# (system/fvSolution is the hex default) and restore it on exit.
...
for N in "${RESOLUTIONS[@]}"; do
    echo "=== tet N=$N (lc=1/$N) ==="
    OUT_DIR="studies/results/$N"
    mkdir -p "$OUT_DIR"
    ./Allclean

    # 1) build the gmsh geometry at this characteristic length
    LC="$("$PY" -c "print(1.0/$N)")"
    sed "s/__LC__/$LC/" studies/mesh/tet/box.geo.template > studies/mesh/tet/box.geo

    # 2) mesh with gmsh (legacy msh2) and import into OpenFOAM
    gmsh -3 studies/mesh/tet/box.geo -o box.msh -format msh2 > log.gmsh 2>&1
...
"$PY" studies/mesh/tet/summarize_tet.py studies/results \
    --resolutions "${RESOLUTIONS[@]}" \
    --out studies/results/summary.csv
