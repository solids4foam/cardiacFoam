...
# ============================================================
# Monodomain pseudo-ECG manufactured-solution regression test
# ============================================================

ALLRUN_LOGFILE="log.Allrun"
BLOCKMESH_LOGFILE="log.blockMesh"
EXPECTED_ELECTRODES=(E1 E2 E3 E4 E5)
REF_FILE="regression/monodomainPseudoECG.reference"
...
{
    # The verifier writes manufacturedPseudoECGSummary_<domainName>.dat, where
    ...
    findFirstMatch 'postProcessing/manufacturedPseudoECGSummary_*.dat' \
        || findFirstMatch 'processor*/postProcessing/manufacturedPseudoECGSummary_*.dat'
}
...
checkReferenceValues()
{
    ...
    echo "Monodomain pseudo-ECG reference comparison: ${checks} checks, ${failures} failures"
    return "${failures}"
}
...
checkPseudoECGHeader()
{
    ...
    do
        if ! awk -v electrode="${electrode}" '
            ...
            END { exit !found }
        ' "${dataFile}"; then
            echo "FAIL: electrode ${electrode} missing from pseudoECG header in ${dataFile}"
            return 1
        fi
    done

    echo "PASS: pseudoECG output header contains E1 E2 E3 E4 E5"
    return 0
}
...
if ! awk '
    ...
    END { exit !found }
' "${pseudoECGFile}"; then
    echo "FAIL: no numeric pseudoECG samples found in ${pseudoECGFile}"
    exit 1
fi

echo "PASS: pseudoECG output detected in ${pseudoECGFile}"
...
checkReferenceValues "${errorFile}" "${pseudoECGFile}"
echo
echo "============================================================"
