...
for scheme in $SCHEMES; do
  set_grad "$scheme"
  for N in $RESOLUTIONS; do run_one "$scheme" "$N"; done
done
set_grad "leastSquares"   # restore tutorial default
./Allclean >/dev/null 2>&1
echo "=== scheme_study.csv ==="; cat setup/results/scheme_study.csv

if [ "$RESOLUTIONS" = "10 20 40 80" ] && [ "$SCHEMES" = "GaussLinear leastSquares" ]; then
  _PAPERI_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../../.." && pwd)"
  python3 "$_PAPERI_ROOT/applications/scripts/paperI_results/aggregate.py" eikonal_tet \
      --repo-root "$_PAPERI_ROOT" \
      || echo "WARN: paperI aggregate (eikonal_tet) failed; native output untouched" >&2
else
  echo "NOTE: partial sweep -- canonical Paper I CSV left untouched." >&2
fi
