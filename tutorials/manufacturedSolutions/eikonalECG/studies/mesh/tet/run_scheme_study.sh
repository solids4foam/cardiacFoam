...
cp studies/mesh/tet/fvSolution system/fvSolution
...
# for the monodomain path in ../../../../monodomainPseudoECG/studies/mesh/tet/run_scheme_study.sh.
...
run_one(){ # $1 tag $2 N
  ...
  LC=$($PY -c "print(1.0/$2)"); sed "s|__LC__|$LC|" studies/mesh/tet/box.geo.template > studies/mesh/tet/box.geo
  gmsh -3 studies/mesh/tet/box.geo -o box.msh -format msh2 >/dev/null 2>&1; gmshToFoam box.msh >/dev/null 2>&1; rm -f box.msh
  ...
  cp log.cf "studies/results/logs/${1}_N${2}.log"
  ITERS="$(grep -c '^PIMPLE: iteration' log.cf || true)"
  if [[ "$ITERS" -ge 2500 ]]; then
    echo "WARNING: $1 N=$2 hit the nOuterCorrectors cap (2500) -- outer loop may not have converged, see studies/results/logs/${1}_N${2}.log" >&2
  fi
  if [[ $RC -ne 0 || ! -f postProcessing/manufacturedEikonalActivationTime.dat ]]; then
    echo "SKIPPING $1 N=$2: cardiacFoam exit=$RC or no verifier output (see studies/results/logs/${1}_N${2}.log)" >&2
    return
  fi
  M=$(activation_metrics); E=$(ecg_metrics)
  echo "$1,$2,${M// /,},${E// /,},${ITERS}" >> studies/results/scheme_study.csv
  echo "done $1 N=$2 : activationTime[$M] ecg[$E] outerIters=$ITERS"
}
echo "scheme,N,dx,activationTime_L2,activationTime_Linf,ecg_L2,ecg_Linf,outerIterations" > studies/results/scheme_study.csv
...
./Allclean >/dev/null 2>&1
echo "=== scheme_study.csv ==="; cat studies/results/scheme_study.csv
