...
findManufacturedErrorFile()
{
    ...
    if [ -n "${candidate}" ] && grep -Eq 'Bath-bidomain manufactured solution error summary|Manufactured bath ECG summary' "${candidate}"; then
        echo "${candidate}"
        return 0
    fi
    ...
    do
        if [ -s "${candidate}" ] && grep -Eq 'Bath-bidomain manufactured solution error summary|Manufactured bath ECG summary' "${candidate}"; then
            echo "${candidate}"
            return 0
        fi
    done
...
checkManufacturedErrorSummary()
{
    ...
    if ! grep -Eq 'Field     L1-error|# field L1 L2 Linf|field_L1' "${errorFile}"; then
