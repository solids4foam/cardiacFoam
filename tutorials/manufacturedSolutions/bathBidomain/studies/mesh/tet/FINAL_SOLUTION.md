...
`studies/interfaceStudy/STAGE_REPORT.md`.
...
```bash
RESOLUTIONS="10 20 40" bash studies/run_matched_serial_sweep.sh
```
...
ASSEMBLY=matchedSubmesh \
bash studies/run_parallel_equivalence.sh
```

Final numerical and exact CSVs are under
`studies/interfaceStudy/matchedSubmesh/distanceWeightedHarmonic/N80/`. Frozen
mesh hashes are under `studies/interfaceMeshBank/`; the reproducible compressed
