...
sed "s/__LC__/${LC}/" studies/mesh/tet/three_domain_box.geo.template \
    > studies/mesh/tet/three_domain_box.geo
...
python3 studies/mesh/tet/verify_mesh.py "$CASE_DIR" > "$OUT_DIR/mesh_manifest.txt"

cp log.gmsh log.gmshToFoam log.checkMesh log.checkMesh.strict "$OUT_DIR/"
cp studies/mesh/tet/three_domain_box.geo "$OUT_DIR/"
