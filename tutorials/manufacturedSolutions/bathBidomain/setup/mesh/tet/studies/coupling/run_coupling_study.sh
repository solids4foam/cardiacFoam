...
do
    ...
    rsync -a \
        ...
        --exclude '/setup/mesh/tet/studies/coupling/results/' \
        --exclude '/setup/mesh/tet/interfaceStudy/' \
        --exclude '/setup/mesh/tet/interfaceMeshBank/' \
        --exclude '/setup/mesh/tet/matchedSubmeshStudy/' \
        --exclude '/setup/mesh/tet/results/' \
        "$CASE_DIR/" "$run_case/"
    ...
    cp "$CASE_DIR/setup/mesh/tet/electroProperties" "$run_case/constant/electroProperties"
    cp "$CASE_DIR/setup/mesh/tet/fvSchemes" "$run_case/system/fvSchemes"
    foamDictionary "$run_case/system/controlDict" -entry writeFormat -set ascii >/dev/null

    lc="$($PYTHON -c "print(1.0/float('$n'))")"
    sed "s/__LC__/$lc/" "$CASE_DIR/setup/mesh/tet/three_domain_box.geo.template" \
        > "$run_case/setup/mesh/tet/three_domain_box.geo"
    (
        cd "$run_case"
        ...
        "$PYTHON" setup/mesh/tet/verify_mesh.py "$run_case" > log.verifyMesh 2>&1
    )
