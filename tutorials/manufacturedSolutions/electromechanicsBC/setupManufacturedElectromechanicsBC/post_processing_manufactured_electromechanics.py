...
def plot_summary_dashboard(rows, *, save_path: Path, show: bool = False) -> Path | None:
    ...
    fig.suptitle(
        ...
        "Vm is a rigorous MMS target; D, lambda, and Ta are BC-constrained diagnostics.",
        fontsize=12,
    )
    finalize_matplotlib_figure(fig, save_path=save_path, show=show)
    return save_path
...
def run_postprocessing(*, output_dir: str, setup_root: str | None = None, **_: object) -> list[dict]:
    ...
    print(
        "\nElectromechanics convergence note: Vm is the rigorous manufactured field; "
        "D, lambda, and Ta are reported as BC-constrained coupled diagnostics."
    )
