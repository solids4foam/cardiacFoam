...
sed "s/__LC__/${LC}/" setup/three_domain_box.geo.template \
    > setup/three_domain_box.geo
...
python3 setup/verify_mesh.py "$CASE_DIR" > "$OUT_DIR/mesh_manifest.txt"

cp log.gmsh log.gmshToFoam log.checkMesh log.checkMesh.strict "$OUT_DIR/"
cp setup/three_domain_box.geo "$OUT_DIR/"
