...
Validates the `ionicHeterogeneity` parameter map for any heterogeneity-capable
ionic model by sweeping synthetic transmural-distance samples and checking that
the resulting action potentials are physiologically consistent and vary smoothly
...
3. Constructs `nSamples` integration points uniformly distributed over the
   ...
   parameter map via the selected heterogeneity mode.
...
5. Writes outputs to `postProcessing/ionicHeterogeneityProbe/`:
   ...
   - `APD_envelope_report.csv` — per-sample APD envelope check (optional)
...
or if APD envelope checks fail (when enabled and `failOnAPDEnvelope` is true).
...
- **`namedRegions`**: Uses an open, user-defined list of named field regions
  ...
  derived from region boundaries (first region boundary → endo–M interface,
  penultimate region boundary → M–epi interface), but can be overridden via
  probe dictionary keys.

- **`cellZoneRegions`** (unsupported): Will raise a fatal error. This mode
  assigns regions via cell zones (no continuous distance field to sweep), making
  it incompatible with the probe's transmural-sweep approach.
