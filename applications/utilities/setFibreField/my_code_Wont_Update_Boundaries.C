...
int main(int argc, char *argv[])
{
    ...
    volScalarField v
      (
       ...
       mesh,
       dimensionedScalar("v", dimless, 0.0)                    
       );
    ...
    // Write u and v after updating boundary values
    Info << "Writing u" << endl;
    u.write();
    Info << "Writing v" << endl;
    v.write();
    ...
    forAll(et, cellI) {
      ...
      scalar dx_du_z = rs[cellI] * Foam::cos(u[cellI]) * Foam::sin(v[cellI]); // 𝑟𝑠 * sin(u) * cos(v)
      scalar dx_du_y = rs[cellI] * Foam::cos(u[cellI]) * Foam::cos(v[cellI]); // 𝑟𝑠 * sin(u) * sin(v)
      scalar dx_du_x = -rl[cellI] *Foam::sin(u[cellI]);  // 0
	...
	f1[cellI] = ( Foam::sin(alphaRadians[cellI]) * (dx_du[cellI]/norm_dx_du)) + ( Foam::cos(alphaRadians[cellI]) * (dx_dv[cellI]/norm_dx_dv));
