...
class Mesh:
    ...
    transmural: Optional[np.ndarray] = None
    transmural_max: Optional[float] = None
    ...
    def __post_init__(self):
        ...
        if self.transmural is not None:
            self.transmural = np.array(self.transmural)
        ...
        for o in order:
            ...
            if sign_r >= 0 and sign_t >= 0:
                ...
                if r <= 1 and t <= 1 and (r + t) <= 1.001:
                    if self.transmural is not None and self.transmural_max is not None:
                        a = 1.0 - r - t
                        idx0, idx1, idx2 = self.connectivity[i]
                        value = (
                            a * self.transmural[idx0]
                            + r * self.transmural[idx1]
                            + t * self.transmural[idx2]
                        )
                        if value > self.transmural_max:
                            continue
