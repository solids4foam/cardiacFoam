...
class Branch:
    ...
    @classmethod
    def set_debug_grow(cls, enabled: bool) -> None:
        cls.debug_grow = enabled

    @classmethod
    def set_debug_dir(cls, path: str) -> None:
        cls.debug_dir = path

    @classmethod
    def set_debug_every(cls, every: int) -> None:
        cls.debug_every = max(1, int(every))

    @classmethod
    def _write_debug_attempt(cls, start: np.ndarray, target: np.ndarray) -> None:
        os.makedirs(cls.debug_dir, exist_ok=True)
        cls.debug_step += 1
        points = np.vstack([start, target])
        cells = [("line", np.array([[0, 1]]))]
        mesh = meshio.Mesh(points=points, cells=cells)
        out_path = os.path.join(cls.debug_dir, f"attempt_{cls.debug_step:06d}.vtu")
    ...
    def _grow(
        ...
        step_vec = self.dir * length / num_segments
        start = self.queue[i - 1]
        target = start + step_vec
        if self.debug_grow:
            logger.info(
                "Grow step %d/%d target=%s",
                i,
                num_segments - 1,
                np.array2string(target, precision=6),
            )
            if (i % self.debug_every) == 0:
                self._write_debug_attempt(start, target)
        intriangle = self.add_node_to_queue(mesh, start, step_vec)
        if not intriangle:
            if self.debug_grow:
                logger.info("  -> projection failed (point not in surface triangle)")
            self.growing = False
            return

        collision = nodes.collision(self.queue[i])
        seg_len = np.linalg.norm(step_vec)
        if collision[1] < 0.8 * seg_len:

            if self.debug_grow:
                logger.info(
                    "  -> collision at step %d: node=%s dist=%.6g",
                    i,
                    np.array2string(np.atleast_1d(collision[0]), precision=6),
                    collision[1],
                )
            self.growing = False
            self.queue.pop()
