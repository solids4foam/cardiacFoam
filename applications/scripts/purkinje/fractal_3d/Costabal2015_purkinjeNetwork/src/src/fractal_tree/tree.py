...
def save_tree(
    ...
    ####Added saving variable for fractal generation 
def save_tree_frame(
    vtu_path: Path,
    nodes: Nodes,
    lines: list[tuple[int, int]],
    ...
    """
    Save the current (partially grown) tree as a VTU file.
    One call = one animation frame.
    """
    xyz = np.zeros((len(nodes.nodes), 3))
    for i in range(len(nodes.nodes)):
        xyz[i, :] = nodes.nodes[i]

    write_line_VTU(xyz, lines, vtu_path)
...
def generate_fractal_tree(
    ...
    # ---- FRAME OUTPUT SETUP ----
    save_frames = getattr(parameters, "save_frames", False)

    if save_frames:
        frames_dir = Path("outputs") / "videoEditorPurkinje" / f"frames_{Path(parameters.filename).name}"
        frames_dir.mkdir(parents=True, exist_ok=True)

        def frame_path(step: int) -> Path:
            base = Path(parameters.filename).name
            return frames_dir / f"{base}_step_{step:03d}.vtu"

        # frame 0 = initial trunk (+ fascicles)
        save_tree_frame(frame_path(0), nodes, lines)

    # ---- MAIN GENERATION LOOP ---- 
    for it in tqdm.tqdm(range(parameters.N_it)):
        ...
        if save_frames:
            save_tree_frame(frame_path(it + 1), nodes, lines)
