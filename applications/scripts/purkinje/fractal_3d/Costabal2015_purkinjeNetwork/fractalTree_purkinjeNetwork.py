...
def meshio_from_pyvista(surface):
    ...
    point_data = {name: np.asarray(arr) for name, arr in surface.point_data.items()}
    return meshio.Mesh(
...
verts = msh.points
transmural_field = getattr(cfg, "transmural_field", "uvc_transmural")
transmural_max = getattr(cfg, "transmural_max", None)
transmural = None
if hasattr(msh, "point_data") and transmural_field in msh.point_data:
    transmural = np.asarray(msh.point_data[transmural_field]).reshape(-1)
elif transmural_max is not None:
    print(
        f"WARNING: transmural_field '{transmural_field}' not found; "
        "transmural constraint disabled."
    )
...
def prune_surface_vertices(points, conn, label, point_data=None):
    used = np.unique(conn)
    ...
    if point_data is not None:
        pruned_data = point_data[used]
    print(
        f"{label}: pruned {points.shape[0] - used.size} unused points "
        f"(kept {used.size})."
    )
    return pruned_points, pruned_conn, pruned_data


surfaces = {}
transmural_by_surface = {}
for name, required in (("ENDO_LV", True), ("ENDO_RV", True), ("EPI", False)):
    ...
    surf_points, surf_conn, surf_trans = prune_surface_vertices(
        verts, conn, name, transmural
    )
    surfaces[name] = (surf_points, surf_conn)
    transmural_by_surface[name] = surf_trans
    print(f"{name} ({tag_key}): triangles = {surf_conn.shape[0]}")

lv_verts, conn_lv = surfaces["ENDO_LV"]
rv_verts, conn_rv = surfaces["ENDO_RV"]
lv_transmural = transmural_by_surface.get("ENDO_LV")
rv_transmural = transmural_by_surface.get("ENDO_RV")
...
mesh_lv = Mesh(
    verts=lv_verts,
    connectivity=conn_lv,
    init_node=seed_lv,
    transmural=lv_transmural,
    transmural_max=transmural_max,
)
mesh_rv = Mesh(
    verts=rv_verts,
    connectivity=conn_rv,
    init_node=seed_rv,
    transmural=rv_transmural,
    transmural_max=transmural_max,
)
...
frames_root = OUTPUT_DIR / "videoEditorPurkinje"
