...
#include "NiedererHunterSmith.H"
#include "NiedererHunterSmith_2006.H"
#include "activeTensionIO.H"
#include "addToRunTimeSelectionTable.H"

namespace Foam
{
    defineTypeNameAndDebug(NiedererHunterSmith, 0);
    addToRunTimeSelectionTable(activeTensionModel, NiedererHunterSmith, dictionary);
}


// * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * * //

Foam::NiedererHunterSmith::NiedererHunterSmith
(
...
{
    Info<< nl << "Initialize NiedererHunterSmith constants:" << nl;
...
void Foam::NiedererHunterSmith::calculateTension
(
    const scalar t,
    const scalar dt,
...
)
{
    ...
    forAll(STATES_, integrationPtI)
    {
        ...
        NiedererHunterSmithcomputeVariables
        (
            tEnd,
            CONSTANTS_.data(),
            RATESI.data(),
...
void Foam::NiedererHunterSmith::derivatives
(
    const scalar t,
    const scalarField& y,
    scalarField& dydt
...
void Foam::NiedererHunterSmith::jacobian
(
    const scalar,
    const scalarField&,
    scalarField&,
...
{
    notImplemented("Foam::NiedererHunterSmith::jacobian(...)");
}

void Foam::NiedererHunterSmith::debugPrintFields
(
    const label cellI,
    const scalar t1,
    const scalar t2,
...
) const
{
    ...
    {
        availableNames[i] = NiedererHunterSmithSTATES_NAMES[i];
    ...
    for (label i = 0; i < NUM_ALGEBRAIC; ++i)
    {
        ...
        availableNames[idx] = NiedererHunterSmithALGEBRAIC_NAMES[i];
...
Foam::wordList Foam::NiedererHunterSmith::availableFieldNames() const
{
    wordList names(NUM_STATES + NUM_ALGEBRAIC);

    for (label i = 0; i < NUM_STATES; ++i)
    {
        names[i] = NiedererHunterSmithSTATES_NAMES[i];
    }

    for (label i = 0; i < NUM_ALGEBRAIC; ++i)
    {
        names[NUM_STATES + i] = NiedererHunterSmithALGEBRAIC_NAMES[i];
    }

    return names;
}

void Foam::NiedererHunterSmith::fieldValues
(
    const label i,
    scalarField& values
) const
