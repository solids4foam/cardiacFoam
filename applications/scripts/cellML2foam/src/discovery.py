...
#
# Description
#     Automates discovery of cardiac model components from Myokit.
#
# Author
...
def find_tissue(model):
    """Heuristic logic to find the tissue type variable."""
    # 1. Check labels
    labels = ["cell_type", "tissue_type"]
    for label in labels:
        v = find_variable_by_label(model, label)
        if v:
            return v
    ...
    # 2. Check common names
    for v in model.variables():
        name = v.name().lower().replace(".", "_").replace("_", "")
        if name in ["celltype", "tissuetype", "tissueflag"]:
            return v
...
def discover_all(model):
    ...
    t = find_tissue(model)
    if t:
        results["Tissue"] = t.name().replace(".", "_")
