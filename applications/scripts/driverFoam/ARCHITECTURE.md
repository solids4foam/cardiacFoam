...
| `core/runtime/generic_case.py` | `make_spec()` parameters: `electro_property_overrides`, `physics_property_overrides` | C — Project-specific | **Yes** | No | These are cardiacFoam dictionary names; the generic factory should not name them |
| `core/runtime/generic_case.py` | `electro_properties_relpath`, `physics_properties_relpath` parameters | C — Project-specific | **Yes** | No | Same; should be `dictionary_overrides: dict[str, Overrides]` or removed |
...
| `core/runtime/generic_case.py::make_spec()` | Parameters `electro_property_overrides`, `physics_property_overrides`, `electro_properties_relpath`, `physics_properties_relpath` | Historical: generic factory was originally the cardiac factory | **Actual coupling** — a non-cardiac project sees cardiac dictionary names in the generic API |
