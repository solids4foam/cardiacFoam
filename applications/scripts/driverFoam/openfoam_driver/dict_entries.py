...
ELECTRO_PROPERTY_ENTRY_GROUPS: Final[dict[str, tuple[DictEntry, ...]]] = {
    ...
    "ionic_constant_overrides": build_group(
        ...
        DictEntry(
            ...
            description="Scale an ionic model constant by a multiplicative factor. Applied globally (every cell). Use for drug effects, channelopathies, or ischaemia: e.g. AC_g_Kr 0.5 halves IKr conductance (LQT2/hERG block). Constant names have the 'AC_' prefix and are listed in ionic_model_catalog.py under the model's 'constants' field. scale is applied before set; using both on the same constant is an error.",
            notes='Common TNNP examples: AC_g_Kr (IKr/hERG), AC_g_Ks (IKs/KCNQ1), AC_g_Na (INa/SCN5A), AC_g_CaL (ICaL/CACNA1C), AC_g_K1 (IK1). ToRORd_dynCl uses the same AC_ prefix; check the catalog for exact names.',
            value_kind='scalar',
            dynamic_path=True,
        ),
        DictEntry(
            driver_path='$ELECTRO_MODEL_COEFFS.ionicConstantOverrides.global.set.<AC_name>',
            description='Set an ionic model constant to an absolute value, overriding the hardcoded default entirely. Applied globally (every cell). Use when a precise literature value is known and scaling from the default would be unreliable. set is applied after scale; using both on the same constant is an error.',
            value_kind='scalar',
            dynamic_path=True,
        ),
        ),
    ...
    "monodomain": build_group(
        ...
        DictEntry(
            ...
            source_refs=('src/electroModels/myocardiumModels/monodomainSolver/monodomainSolver.C', 'src/electroModels/myocardiumModels/eikonalSolver/eikonalSolver.C'),
    ...
    "bath_potential_domain": build_group(
        ...
        DictEntry(
            driver_path='$ELECTRO_MODEL_COEFFS.bathPdeCouplingMethod',
            source_refs=('src/electroModels/core/advanceSchemes/staggered/staggeredElectrophysicsAdvanceScheme.C',),
            phases=frozenset({'solver'}),
            description="Coupling used to advance the myocardium Vm and the global extracellular potential phiE within one step. onePass (default) solves phiE once from the post-reaction Vm, then applies the PIMPLE loops to Vm — the historical algorithm. predictorCorrector predicts Vm, updates phiE from that predictor, then corrects Vm (a block Gauss-Seidel coupling); neither path advances physical time twice.",
            value_kind='enum',
            enum_values=('onePass', 'predictorCorrector'),
            typical_value='onePass',
            constraints=('Only applicable to bidomainSolver runs with a bathPotentialDomain configured.',),
        ),
        ),
    ),
    "eikonal_diffusion": build_group(
        defaults={"source_refs": ("src/electroModels/myocardiumModels/eikonalSolver/eikonalSolver.C",)},
        entries=(
        DictEntry(
    ...
    "ecg": build_group(
        ...
        DictEntry(
            ...
            phases=frozenset({'physics'}),
            description='List of quadrature orders used to compare manufactured pseudo-ECG reference convergence. Default when omitted: a single-element list [6].',
            source_refs=('src/verificationModels/ecgVerification/pseudoECGManufacturedVerifier.C',),
    ...
    "active_tension": build_group(
        ...
        DictEntry(
            driver_path='$ELECTRO_MODEL_COEFFS.TaScale',
            description='Scales the active tension magnitude computed by the model. Flat sibling of activeTensionModel inside <solver>Coeffs, not nested under it.',
    ...
    "domain_couplings": build_group(
        ...
        DictEntry(
            ...
            description="Coupling direction: 'unidirectional' (Purkinje→myocardium only) or 'bidirectional' (both ways). Default: unidirectional.",
