...
tmp<volTensorField> eikonalDiffusionElectro::initialiseConductivity() const {
  ...
  tmp<volTensorField> tresult(new volTensorField(
      IOobject("conductivity", runTime().timeName(), mesh(),
  ...
  if (!result.headerOk()) {
    ...
    result = dimensionedTensor(
        dimensionedSymmTensor("conductivity",
                              pow3(dimTime) * sqr(dimCurrent) /
                                  (dimMass * dimVolume),
                              electroProperties()) &
        tensor(I));
...
eikonalDiffusionElectro::eikonalDiffusionElectro(Time &runTime,
                                                 const word &region)
    : electroModel(typeName, runTime, region),
      psi_(IOobject("psi", runTime.timeName(), mesh(),
                    IOobject::READ_IF_PRESENT, IOobject::AUTO_WRITE),
           mesh(), dimensionedScalar("psi", dimTime, 0.0), "zeroGradient"),
      Vm_(IOobject("Vm", runTime.timeName(), mesh(), IOobject::READ_IF_PRESENT,
                   IOobject::AUTO_WRITE),
          mesh(), dimensionedScalar("Vm", dimVoltage, -80.0), "zeroGradient"),
      gradPsi_(fvc::grad(psi_)), stimulusCellIDs_(0),
      ...
      M_("M", conductivity_ / (chi_ * Cm_)), w_("w", M_ & gradPsi_),
      G_("G",
         sqrt((gradPsi_ & w_) + dimensionedScalar("smallG", dimTime, SMALL))),
      c0_("c0", electroProperties()), a_("a", w_ / G_), u_("u", c0_ * a_),
  ...
  const boundBox bb(point(electroProperties().lookup("stimulusLocationMin")),
  ...
  Info << "Surface-to-volume ratio chi = " << chi_ << nl
       << "Membrane capacitance Cm = " << Cm_ << nl << "M tensor field:" << nl
       << "    max(M) = " << gMax(M_) << nl << "    min(M) = " << gMin(M_) << nl
...
bool eikonalDiffusionElectro::evolve() {
  ...
  while (pimple().loop()) {
    ...
    if (eikonalAdvectionDiffusionApproach_) {
      ...
      fvScalarMatrix psiEqn(-fvm::laplacian(M_, psi_) + fvm::div(phiU_, psi_) +
                                fvm::SuSp(-divPhiU_, psi_) ==
                            one + fvc::div(phiU_, psi_) - divPhiU_ * psi_ -
                                c0_ * G_);
      ...
      fvScalarMatrix psiEqn(-fvm::laplacian(M_, psi_) + c0_ * G_ == one);
