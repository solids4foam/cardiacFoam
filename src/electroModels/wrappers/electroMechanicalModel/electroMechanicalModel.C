...
Foam::electroModels::electroMechanicalModel::New
(
    Time& runTime,
    const word& region
)
...
:
    ...
    TaPtr_(),
    lambdaPtr_(),
    activeTensionOutFields_(),
    activeTensionOutputPtr_()
{
    ...
    if (props.found("activeTensionModel"))
    {
        ...
        // Merge parent dict so ODE solver keys are available in the
        // sub-dictionary without duplicating them in the case.
        ...
        const ElectromechanicalSignalProvider* provider =
            electroModelPtr_->provider();
        if (provider)
        {
            activeTensionModelPtr_->setElectromechanicalSignalProvider
            (
                *provider
            );
        }
        ...
        (
            ...
            (
                ...
                electroModelPtr_->mesh(),
                dimensionedScalar("Ta", dimPressure, 0.0),
                "zeroGradient"
            )
        );
...
bool Foam::electroModels::electroMechanicalModel::evolve()
{
    ...
    if (activeTensionModelPtr_.valid())
    {
        ...
        if (activeTensionOutputPtr_.valid())
        {
            ...
            if
            (
                ionicModelIO::shouldWriteStep
                (
                    t0,
                    t1,
                    electroModelPtr_->electroProperties(),
                    false
                )
            )
            {
...
{
    const bool status = electroModelPtr_->read();
