...
void purkinjeNetworkModel::openOutputFile()
{
    ...
    outputPtr_ = ecgModelIO::openTimeSeries
    (
...
:
    GraphConductionSystemDomain(context, dict),
    supportMesh_(context.supportMesh()),
    graph_(),
    pvjNodes_(),
    pvjLocations_(),
    pvjCellIDs_(),
    ...
    debugVars_()
{
    ...
    // The ionic model manages its own internal resting state independently
    // (e.g. BuenoOrovio: u=0, Stewart: -69.14 mV), so no model-specific
    // value is needed here — the conversion inside each ionicModel::solveODE
    // maps -0.084 V to the correct dimensionless/physical resting state.
    const scalar Vm1D_rest =
        dict.subDict("purkinjeNetworkModelCoeffs")
            .getOrDefault<scalar>("Vm1D_rest", -0.084);
...
void purkinjeNetworkModel::assembleAppliedCurrent
(
    scalar t0,
...
void purkinjeNetworkModel::reportAdvanceDiagnostics
(
    scalar          t0,
    scalar          dt
) const
...
void purkinjeNetworkModel::terminalVm(scalarField& values) const
{
...
void purkinjeNetworkModel::setTerminalCoupling
(
...
void purkinjeNetworkModel::write()
{
    ...
    if (Pstream::master())
    {
        ecgModelIO::writeRow(outputPtr_.ref(), time().value(), vals);
    }
}
